...
int APIENTRY wWinMain(_In_ HINSTANCE instance, _In_opt_ HINSTANCE prev,
  ...
  for (const auto& arg : command_line_arguments) {
    // desktop_multi_window passes "multi_window" as the first argument
    if (arg == "multi_window") {
      is_sub_window = true;
      break;
    }
  ...
  // Manager.
  if (!is_sub_window && !AcquireSingleInstance()) {
    return EXIT_SUCCESS;
  }
  ...
  // Lower the priority of the entire process to ensure that the game process has absolute priority under high load
