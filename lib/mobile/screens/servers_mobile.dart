...
class _ServersMobileState extends State<ServersMobile>
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    context.read<ServerBloc>().add(ServerLifecycleChanged(state == AppLifecycleState.resumed));
  }
            ...
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  ElevatedButton(
                    onPressed: () => context.read<ServerBloc>().add(ServerFetchList()),
                    child: const Text('重试'),
                  ),
                ],
              ),
          ...
          child: Padding(
            ...
            child: Row(
              children: [
                ...
                Expanded(child: _buildTitleSection(theme, colorScheme, title, subtitle, state)),
  ...
  Widget _buildBackButton(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: const Color(0xFF0080FF).withValues(alpha: 0.2), width: 1),
      ),
      child: IconButton(
        icon: const Icon(Icons.arrow_back_rounded, color: Color(0xFF0080FF), size: 20),
        onPressed: () {
    ...
    ).animate().scale(begin: const Offset(0.8, 0.8), duration: 400.ms, curve: Curves.elasticOut).fadeIn(duration: 300.ms);
  }

  Widget _buildTitleSection(ThemeData theme, ColorScheme colorScheme, String title, String subtitle, ServerState state) {
    return Row(
      children: [
        Container(
          width: 56,
            ...
            decoration: BoxDecoration(
              gradient: LinearGradient(
                colors: state.selectedCategory != null 
                    ? [
                        CategoryUtils.getCategoryColor(state.selectedCategory?.modelName),
                        CategoryUtils.getCategoryColor(state.selectedCategory?.modelName).withValues(alpha: 0.8),
                      ]
            ...
            child: AnimatedSwitcher(
              ...
              child: Icon(
                state.selectedCategory != null
                    ? CategoryUtils.getCategoryIcon(state.selectedCategory?.modelName)
  ...
  Widget _buildCountdownProgress(ServerState state) {
    ...
    ).animate().scale(begin: const Offset(0.8, 0.8), duration: 500.ms, delay: 200.ms).fadeIn(duration: 400.ms, delay: 200.ms);
  }
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          if (_searchController.text.isNotEmpty) {
            ...
            if (!serverName.contains(searchTerm) && !mapName.contains(searchTerm)) {
