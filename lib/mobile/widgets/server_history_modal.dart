...
class _ServerHistoryModalState extends State<ServerHistoryModal>
  ...
  Future<void> _loadHistoryData({bool refresh = false}) async {
    ...
    try {
      ...
      final response = await _serverApi.getServerHistory(
        address: address,
      ...
      setState(() {
        ...
        isLoading = false;
        hasMoreData =
            newData.length >= pageSize && historyData.length < totalRecords;
      });
  ...
  Future<void> _loadMoreData() async {
    ...
    try {
      ...
      setState(() {
        ...
        isLoadingMore = false;
        hasMoreData =
            moreData.length >= pageSize && historyData.length < totalRecords;
      });
      ...
      if (mounted) {
        ToastUtils.showError(
          context,
          ErrorUtils.getErrorMessage(e, defaultMessage: '加载更多数据失败'),
        );
      }
    }
  }
  ...
  Future<void> _preloadMapInfo(List<ServerSnapshot> snapshots) async {
    final uniqueMapNames = snapshots
        .map((s) => s.mapName)
        .where((name) => name.isNotEmpty && !_mapInfoCache.containsKey(name))
        .toSet();
    for (final mapName in uniqueMapNames) {
      try {
        final mapInfo = await _serverApi.getMapInfo(mapName);
        if (mapInfo != null && mounted) {
          setState(() => _mapInfoCache[mapName] = mapInfo);
        }
      } catch (_) {}
    }
  }
  ...
  void _toggleCardExpansion(int index) =>
      setState(() => _expandedCards[index] = !(_expandedCards[index] ?? false));
  void _performSearch() {
    setState(() {
      currentPage = 1;
      hasMoreData = true;
  ...
  Widget _buildContent() {
    ...
    final filteredData = _searchController.text.isEmpty
        ? historyData
        : historyData.where((snapshot) {
            final searchTerm = _searchController.text.toLowerCase();
            return snapshot.mapName.toLowerCase().contains(searchTerm) ||
                snapshot.mapLabel.toLowerCase().contains(searchTerm);
          }).toList();
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              totalRecords == 0 ? '暂无历史记录' : '未找到匹配的历史记录',
              style: TextStyle(
                fontSize: 16,
                color: colorScheme.onSurfaceVariant,
              ),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              Formatters.formatDateTime(snapshot.createdAt),
              style: TextStyle(
                fontSize: 13,
        ...
        children: [
            ...
            child: mapInfo?.mapUrl != null
                ? DiskCachedImage(
                    ...
                    errorWidget: Container(
                      ...
                      child: const Center(
                        child: Icon(Icons.map, size: 40, color: Colors.white54),
                      ),
                    ),
                  )
                : Container(
          ...
          Positioned(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                if (mapInfo?.mapLabel != null &&
                    mapInfo!.mapLabel.isNotEmpty &&
                    mapInfo.mapLabel != snapshot.mapName)
                  Text(
                    mapInfo.mapLabel,
          ...
          if (snapshot.hasFinalScore)
            Positioned(
              ...
              child: Builder(
                builder: (context) {
                  final isZombieMap =
                      snapshot.mapName.startsWith('ze_') ||
                      snapshot.mapName.startsWith('zm_');
                  return Container(
  ...
  Widget _buildStatsInfo(ServerSnapshot snapshot) {
    ...
    if (snapshot.infos != null && snapshot.infos!.isNotEmpty) {
      maxHistoryPlayers = snapshot.infos!
          .map((e) => e.playerCount)
          .reduce((a, b) => a > b ? a : b);
    }
    return Padding(
      ...
      child: Row(
        children: [
          _buildStatItem(
            Icons.trending_up,
            maxHistoryPlayers.toString(),
            '最高人数',
          ),
          const SizedBox(width: 20),
          _buildStatItem(
            Icons.access_time,
            _getMapPlayDuration(snapshot),
            '运行时长',
          ),
        ],
      ),
  ...
  Widget _buildPlayerTrendInfo(ServerSnapshot snapshot) {
    ...
    final maxCount = playerInfos
        .map((e) => e.playerCount)
        .reduce((a, b) => a > b ? a : b);
    final minCount = playerInfos
        .map((e) => e.playerCount)
        .reduce((a, b) => a < b ? a : b);
    final avgCount =
        (playerInfos.map((e) => e.playerCount).reduce((a, b) => a + b) /
                playerInfos.length)
            .round();
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(Icons.trending_up, size: 15, color: colorScheme.primary),
          ...
          Row(
            ...
            children: [
              _buildTrendStat('最高', maxCount.toString(), Colors.red),
              _buildTrendStat('平均', avgCount.toString(), Colors.blue),
              _buildTrendStat('最低', minCount.toString(), Colors.green),
            ],
          ),
        ],
      ),
  ...
  String _getMapPlayDuration(ServerSnapshot snapshot) {
    ...
    final sortedInfos = [...snapshot.infos!]
      ..sort((a, b) {
...
class _TrendChartWidgetState extends State<_TrendChartWidget> {
  ...
  String _formatTimeLabel(String dateTimeString) {
    try {
      final date = TimeUtils.parseServerTime(dateTimeString);
  ...
  String _formatFullTime(String dateTimeString) {
    try {
      final date = TimeUtils.parseServerTime(dateTimeString);
      return date != null
          ? DateFormat('yyyy年MM月dd日 HH:mm:ss').format(date)
          : '';
    } catch (_) {
      return '';
    }
                ...
                SizedBox(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        (widget.maxCount * 0.75).round().toString(),
                        style: TextStyle(
                          fontSize: 9,
                          color: colorScheme.onSurfaceVariant,
                        ),
                      ...
                      Text(
                        (widget.maxCount * 0.5).round().toString(),
                        style: TextStyle(
                          fontSize: 9,
                          color: colorScheme.onSurfaceVariant,
                        ),
                      ...
                      Text(
                        (widget.maxCount * 0.25).round().toString(),
                        style: TextStyle(
                          fontSize: 9,
                          color: colorScheme.onSurfaceVariant,
                        ),
                ...
                Expanded(
                  child: GestureDetector(
                    onTapDown: (details) {
                      ...
                      final chartStartX = 12 + 28 + 6;
                      final chartWidth =
                          renderBox.size.width - chartStartX - 12;
                      final chartStartY = 12;
                      final chartHeight = renderBox.size.height - 12 - 6 - 18;
                      if (localPosition.dx >= chartStartX &&
                          localPosition.dx <= chartStartX + chartWidth &&
                          localPosition.dy >= chartStartY &&
                          localPosition.dy <= chartStartY + chartHeight) {
                        final relativeX =
                            (localPosition.dx - chartStartX) / chartWidth;
                        final dataIndex = (relativeX * (displayData.length - 1))
                            .round()
          ...
          if (displayData.isNotEmpty)
            Row(
              ...
              children: [
                ...
                Text(
                  _formatTimeLabel(displayData.first.createdAt),
                  style: TextStyle(
                    fontSize: 9,
                    color: colorScheme.onSurfaceVariant,
                  ),
                ...
                if (displayData.length > 2)
                  Text(
                    _formatTimeLabel(
                      displayData[displayData.length ~/ 2].createdAt,
                    ),
                    style: TextStyle(
                      fontSize: 9,
                      color: colorScheme.onSurfaceVariant,
                ...
                Text(
                  _formatTimeLabel(displayData.last.createdAt),
                  style: TextStyle(
                    fontSize: 9,
                    color: colorScheme.onSurfaceVariant,
                  ),
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < data.length; i++) {
      ...
      final ratio = maxValue > 0
          ? (data[i].playerCount / maxValue).clamp(0.0, 1.0)
          : 0.0;
