...
/// 服务器详情弹窗
/// 显示服务器详细信息和玩家列表
class ServerPlayersDialog extends StatefulWidget {
  final ExtendedServerItem server;

  const ServerPlayersDialog({super.key, required this.server});

  @override
  State<ServerPlayersDialog> createState() => _ServerPlayersDialogState();
}

class _ServerPlayersDialogState extends State<ServerPlayersDialog> {
  // 状态
  ...
  Future<void> _fetchPlayerList() async {
    ...
    try {
      ...
      if (players.isNotEmpty) {
        ...
        if (playerCount > 55) {
          setState(() {
            _playerError = '疑似受底层通信协议单包体积限制，中文名称占用字节较大，超过约55人时极易超出上限导致获取失败';
            _isLoadingPlayers = false;
          });
  ...
  Widget _buildPlayerList() {
    ...
    // 正在加载玩家数据且当前列表为空
    if (_isLoadingPlayers && players.isEmpty) {
      return const Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            ...
            SizedBox(height: 16),
            Text('正在获取玩家列表...', style: TextStyle(color: AppColors.gray500)),
          ],
        ),
      );
    }
