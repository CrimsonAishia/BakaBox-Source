...
class _UnifiedEditDialogState extends State<UnifiedEditDialog>
  ...
  void initState() {
    ...
    final currentDescription = (widget.subModel.description?.isNotEmpty ?? false)
    ...
    final currentAcquisition = widget.subModel.acquisition ?? widget.character.acquisition;
  ...
  /// 从待审核申请中初始化符卡编辑数据
  void _initSpellCardEditsFromPendingRequest(SpellCardsEditData? spellCardsData) {
    ...
    if (spellCardsData.creates != null) {
      for (final create in spellCardsData.creates!) {
        _spellCardCreates.add(SpellCardCreateData(
          name: create.name,
          type: create.type,
  ...
  /// 从待审核申请中初始化僵尸技能编辑数据
  void _initZombieSkillEditsFromPendingRequest(ZombieSkillsEditData? zombieSkillsData) {
    ...
    if (zombieSkillsData.creates != null) {
      for (final create in zombieSkillsData.creates!) {
        _zombieSkillCreates.add(ZombieSkillCreateData(
          name: create.name,
          type: create.type,
          description: create.description,
          damage: create.damage,
  ...
  Widget _buildAcquisitionTab() {
    ...
    final originalAcquisition = widget.subModel.acquisition ?? widget.character.acquisition;
    final isMaintained = originalAcquisition != null && originalAcquisition.type != AcquisitionType.unknown;
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              ...
              _buildAcquisitionTypeChip('点', AcquisitionType.points, vermillion),
              _buildAcquisitionTypeChip('🎁 自定义', AcquisitionType.custom, const Color(0xFF4A7C59)),
              // 只有未维护时才显示"未知"选项
              if (!isMaintained)
                _buildAcquisitionTypeChip('❓ 未知', AcquisitionType.unknown, scrollBrown),
            ],
          ),
  ...
  Widget _buildZombieSkillsTab() {
    final skills = widget.character.zombieSkills
  ...
  Widget _buildSpellCardItem(SpellCard card) {
    ...
    final overlayColor = CharacterGalleryTheme.getOverlayColor(context, alpha: isDark ? 0.6 : 0.8);
    final vermillion = CharacterGalleryTheme.getVermillion(context);
    final gold = CharacterGalleryTheme.getGold(context);
              ...
              child: Column(
                ...
                children: [
                    ...
                    children: [
                      ...
                      if (isEditing) ...[
                        PopupMenuButton<SpellCardTier?>(
                          ...
                          onSelected: (tier) => setState(() => _tempTier = tier),
                          child: Container(
                            padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                            decoration: BoxDecoration(
                              color: overlayColor,
                              border: Border.all(
                                color: currentTier != null && currentTier != SpellCardTier.unranked
                          ...
                          itemBuilder: (context) => [
                            PopupMenuItem(
                              value: null,
                              child: Text('未选择', style: TextStyle(color: inkColor)),
                            ),
                            ...SpellCardTier.values.map((tier) => PopupMenuItem(
                              value: tier,
                              child: Text(
                                tier.label,
                                style: TextStyle(
                        ...
                        const SizedBox(width: 8),
                      ] else if (currentTier != null && currentTier != SpellCardTier.unranked) ...[
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                          decoration: BoxDecoration(
                            color: overlayColor,
                            border: Border.all(color: _getTierColor(currentTier)),
                            borderRadius: BorderRadius.circular(4),
                          ),
                          child: Text(
                      ...
                      // 操作按钮
                      if (isEditing) ...[
                        _buildIconButton(Icons.check, () => _saveInlineSpellCardEdit(card), color: Colors.green),
                        _buildIconButton(Icons.close, _cancelInlineSpellCardEdit, color: Colors.grey),
                      ] else ...[
                        _buildIconButton(Icons.edit_outlined, () => _editSpellCard(card)),
                        _buildIconButton(Icons.delete_outline, () => _deleteSpellCard(card.id), color: Colors.red),
                      ],
                    ],
                  ),
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: isEditing ? overlayColor.withValues(alpha: 0.3) : Colors.transparent,
                    ...
                    child: Stack(
                      children: [
                        ...
                        // 字数统计（仅编辑模式显示）
                        if (isEditing && _tempDescriptionController != null)
                          Positioned(
                            right: 0,
                            bottom: 0,
                            child: Container(
                  ...
                  // 属性行
                  if ((isEditing ? ((_tempCooldownController?.text.isNotEmpty ?? false) || (_tempDamageController?.text.isNotEmpty ?? false) || (_tempCostController?.text.isNotEmpty ?? false)) : ((editData?.cooldown ?? card.cooldown) != null || (editData?.damage ?? card.damage) != null || (editData?.cost ?? card.cost) != null))) ...[
                    const SizedBox(height: 10),
                    if (isEditing)
                      _buildEditableStatsRow(card, borderColor, inkColor, overlayColor)
                    else
  ...
  // 可编辑的属性行
  Widget _buildEditableStatsRow(SpellCard card, Color accentColor, Color inkColor, Color overlayColor) {
    return Wrap(
      spacing: 12,
      runSpacing: 8,
      children: [
        // 冷却时间
        _buildInlineStatField(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      // 梯队标签
                      if (data.tier != null && data.tier != SpellCardTier.unranked) ...[
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                          decoration: BoxDecoration(
                            color: overlayColor,
                            border: Border.all(color: _getTierColor(data.tier!)),
                            borderRadius: BorderRadius.circular(4),
                          ),
                          child: Text(
                            data.tier!.shortLabel,
                            style: TextStyle(
                  ...
                  // 属性行
                  if (data.cooldown != null || data.damage != null || data.cost != null) ...[
                    const SizedBox(height: 10),
                    _buildSpellCardStatsRow(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 8),
                      _buildIconButton(Icons.edit_outlined, () => _editZombieSkill(skill)),
                      _buildIconButton(Icons.delete_outline, () => _deleteZombieSkill(skill.id),
                          color: Colors.red),
                    ],
                  ),
                  ...
                  // 属性行
                  if ((editData?.cooldown ?? skill.cooldown) != null ||
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // 属性行
                  if (data.cooldown != null || data.damage != null || data.range != null) ...[
                    const SizedBox(height: 10),
                    _buildZombieSkillStatsRow(
                      cooldown: data.cooldown,
                      damage: data.damage,
              ...
              BlocConsumer<CharacterGalleryBloc, CharacterGalleryState>(
                ...
                listener: (context, state) {
                  if (state.submitEditState == LoadState.success) {
                    ...
                    ToastUtils.showSuccess(context, widget.isEditMode ? '申请已修改' : '编辑已提交审核');
                  } else if (state.submitEditState == LoadState.failure) {
                    ToastUtils.showError(context, state.submitEditError ?? '提交失败');
                  }
                },
  ...
  Widget _buildAcquisitionTypeChip(String label, AcquisitionType type, Color color) {
  ...
  /// 单个属性项（带阴影，用于符卡）
  Widget _buildStatItemWithShadow(IconData icon, String label, String value, Color color, Color shadowColor) {
    return Row(
      mainAxisSize: MainAxisSize.min,
      children: [
        Icon(
  ...
  void _checkAcquisitionChanged() {
    final original = widget.subModel.acquisition ?? widget.character.acquisition;
    ...
    setState(() {
      _acquisitionChanged = _acquisitionType != originalType ||
  ...
  void _saveInlineSpellCardEdit(SpellCard card) {
    if (_tempDescriptionController == null || _tempDescriptionController!.text.isEmpty) {
    ...
    // 检查是否有实际修改
    final hasChanges = descText != card.description ||
  ...
  void _editZombieSkill(ZombieSkill skill) {
  ...
  void _onSubmit() {
    ...
    // 构建通用的编辑数据
    final description = _descriptionChanged ? _descriptionController.text : null;
    ...
    if (widget.isEditMode) {
      ...
      context.read<CharacterGalleryBloc>().add(UpdateEditRequest(
