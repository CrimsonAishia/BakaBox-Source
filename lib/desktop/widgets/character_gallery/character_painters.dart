...
class TraditionalDividerPainter extends CustomPainter {
  ...
  void _drawSakura(
    ...
    canvas.drawCircle(
      center,
      1.5,
      Paint()..color = CharacterGalleryTheme.goldBright,
    );
  }
...
class _SakuraPetalsPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final petal in petals) {
      final x =
          (petal.startX * size.width + progress * petal.speedX * 100) %
          size.width;
      final y =
          (petal.startY * size.height +
              progress * petal.speedY * size.height * 1.5) %
          size.height;
      final rotation = progress * petal.rotationSpeed * 2 * pi;

      // 夜间模式下降低樱花透明度
      final baseOpacity = isDark ? petal.opacity * 0.6 : petal.opacity;
      paint.color = CharacterGalleryTheme.sakuraPink.withValues(
        alpha: baseOpacity,
      );
