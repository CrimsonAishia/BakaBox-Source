...
/// 摇一摇抽奖提示框
class ShakeTooltipContent extends StatefulWidget {
  final VoidCallback onClose;

  const ShakeTooltipContent({super.key, required this.onClose});

  @override
  State<ShakeTooltipContent> createState() => _ShakeTooltipContentState();
}

class _ShakeTooltipContentState extends State<ShakeTooltipContent>
  ...
  void initState() {
    ...
    // 优先从 DailyTaskBloc 同步获取预加载的状态，避免任何加载闪烁
    final dailyTaskState = context.read<DailyTaskBloc>().state;
    if (!dailyTaskState.isCheckingStatus && dailyTaskState.canShake != null) {
      _isLoading = false;
      if (dailyTaskState.hasShaked) {
        _alreadyShaked = true;
        _hasResult = true;
        _rewardAmount = dailyTaskState.shakeRewardAmount;
        _resultMessage = dailyTaskState.shakeRewardAmount != null
            ? '今日已获得 ${dailyTaskState.shakeRewardAmount} 僵尸币'
            : '今日已摇过，明天再来吧';
  ...
  Future<void> _doShake() async {
    ...
    // 同步捕获 Bloc，防止弹窗关闭导致 context unmounted 从而无法提交数据库
    final dailyTaskBloc = context.read<DailyTaskBloc>();
