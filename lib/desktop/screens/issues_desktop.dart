...
class _IssuesDesktopContentState extends State<_IssuesDesktopContent> {
  ...
  Widget _buildFilters() {
    ...
    return BlocBuilder<IssueBloc, IssueState>(
      buildWhen: (prev, curr) =>
          prev.currentType != curr.currentType ||
    ...
    return Row(
      ...
      children: [
        _buildViewTab(
          '全部',
          !showMine,
          () => context.read<IssueBloc>().add(const IssueSwitchView(false)),
        ),
        ...
        _buildViewTab(
          ...
          showMine,
          isAuthenticated
              ? () => context.read<IssueBloc>().add(const IssueSwitchView(true))
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton.icon(
              onPressed: () =>
                  context.read<IssueBloc>().add(const IssueFetch()),
          ...
          child: Scrollbar(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) =>
                  _buildIssueItem(state.issues[index]),
            ),
          ),
        ),
          ...
          _buildPageButton(
            ...
            onTap: state.currentPage > 1
                ? () => context.read<IssueBloc>().add(
                    IssueGoToPage(state.currentPage - 1),
                  )
                : null,
          ),
          const SizedBox(width: 8),
    ...
    return Padding(
      ...
      child: Material(
        ...
        child: InkWell(
          onTap: isSelected
              ? null
              : () => context.read<IssueBloc>().add(IssueGoToPage(page)),
          borderRadius: BorderRadius.circular(6),
          child: Container(
            width: 32,
            height: 32,
  ...
  Widget _buildIssueItem(IssueListItem issue) {
    return _IssueCard(issue: issue, onTap: () => _navigateToDetail(issue.id));
  }
}
              ...
              Row(
                children: [
                  _buildAuthorInfo(
                    ...
                    widget.issue.authorAvatar,
                    isDark,
                  ),
                  const SizedBox(width: 16),
                  _buildMetaItem(
                    MdiIcons.clockOutline,
                    Formatters.formatRelativeTime(widget.issue.createdAt),
                    isDark,
                  ),
                  const Spacer(),
                  _buildStatItem(
                    MdiIcons.thumbUpOutline,
                    widget.issue.voteCount,
                    isDark,
                  ),
                  const SizedBox(width: 12),
                  _buildStatItem(
                    MdiIcons.commentOutline,
                    widget.issue.commentCount,
                    isDark,
                  ),
                ],
              ),
...
class _IssueDetailViewState extends State<_IssueDetailView> {
  ...
  void _toggleVote() {
    final authState = context.read<AuthBloc>().state;
    if (!authState.isAuthenticated) {
      ToastUtils.showWarning(context, '请先登录后再投票');
      return;
    }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              _buildVoteButton(issue, state),
              const SizedBox(width: 12),
              _buildCommentCountBadge(issue),
              const Spacer(),
              _buildCloseButton(issue, state),
            ],
          ),
        ],
      ),
  ...
  Widget _buildVoteButton(Issue issue, IssueDetailState state) {
    ...
    return OutlinedButton.icon(
      onPressed: state.isSubmitting ? null : _toggleVote,
      icon: Icon(
  ...
  Widget _buildCloseButton(Issue issue, IssueDetailState state) {
    ...
    final isAuthor =
        backendUserInfo != null && backendUserInfo.id == issue.authorId;
  ...
  Widget _buildReplyButton(IssueComment comment, bool isDark) {
      ...
      children: [
        ...
        Row(
          children: IssueType.values.map((type) {
            ...
            final darkBgColor = switch (type) {
              IssueType.bug => const Color(0xFF7F1D1D),
              IssueType.feature => const Color(0xFF1E3A5F),
              IssueType.question => const Color(0xFF064E3B),
            };
            return Padding(
