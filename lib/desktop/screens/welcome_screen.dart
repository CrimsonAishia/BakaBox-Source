...
class _WelcomeScreenState extends State<WelcomeScreen> {
              ...
              builder: (context, serverState) {
                return BlocBuilder<ServerStatsBloc, ServerStatsState>(
                  builder: (context, statsState) {
                    return _buildStatsCards(context, theme, isDark, serverState, statsState);
                  },
                );
              },
            ),
  ...
  /// 构建统计卡片
  Widget _buildStatsCards(BuildContext context, ThemeData theme, bool isDark, ServerState serverState, ServerStatsState statsState) {
    ...
    return Row(
      children: [
        ...
        const SizedBox(width: 16),
        Expanded(flex: 2, child: _PlayerTrendCard(isDark: isDark, delay: 1100, stats: statsState.stats)),
      ],
    );
  }
}
...
class _PlayerTrendCard extends StatefulWidget {
  ...
  const _PlayerTrendCard({required this.isDark, required this.delay, this.stats});
...
class _PlayerTrendCardState extends State<_PlayerTrendCard> {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: widget.isDark ? Colors.white.withValues(alpha: 0.1) : Colors.black.withValues(alpha: 0.05)),
        boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: widget.isDark ? 0.2 : 0.05), blurRadius: 20, offset: const Offset(0, 4))],
      ),
      child: Column(
    ...
    ).animate().fadeIn(duration: 500.ms, delay: widget.delay.ms).slideY(begin: 0.2, end: 0, duration: 400.ms, curve: Curves.easeOutCubic);
  }
  ...
  Widget _buildTabItem(int index, String label) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: isSelected ? const Color(0xFF8B5CF6).withValues(alpha: 0.15) : Colors.transparent,
          borderRadius: BorderRadius.circular(8),
          border: Border.all(color: isSelected ? const Color(0xFF8B5CF6).withValues(alpha: 0.3) : Colors.transparent),
        ),
        child: Text(
          label,
          style: TextStyle(
            ...
            color: isSelected ? const Color(0xFF8B5CF6) : (widget.isDark ? Colors.white.withValues(alpha: 0.5) : const Color(0xFF94A3B8)),
          ),
        ),
      ),
    );
  ...
  Widget _buildTabContent(ServerStatsResponse stats) {
    return switch (_selectedTab) {
      ...
      1 => _HourlyChart(isDark: widget.isDark, hourlyStats: stats.hourlyStats, peakHour: stats.peakHour),
          ...
          children: [
            SizedBox(
              ...
              child: CircularProgressIndicator(
                strokeWidth: 2,
                color: widget.isDark ? Colors.white.withValues(alpha: 0.5) : const Color(0xFF94A3B8),
              ),
            ),
            const SizedBox(height: 12),
            Text(
              '加载统计数据...',
              style: TextStyle(fontSize: 13, color: widget.isDark ? Colors.white.withValues(alpha: 0.5) : const Color(0xFF94A3B8)),
            ),
          ],
        ),
      ),
    ...
    return Column(
      children: [
        Row(
          ...
          children: [
            _LegendItem(color: const Color(0xFF3B82F6), label: '最高 ${stats.weeklyMax}', isDark: isDark),
            const SizedBox(width: 16),
            _LegendItem(color: const Color(0xFF10B981), label: '平均 ${stats.weeklyAvg}', isDark: isDark),
          ],
        ),
        ...
        Expanded(
          child: LineChart(
            LineChartData(
                  ...
                  sideTitles: SideTitles(
                    ...
                    getTitlesWidget: (value, meta) {
                      ...
                      return Padding(
                        ...
                        child: Text('${parts[1]}/${parts[2]}', style: TextStyle(fontSize: 10, color: isDark ? Colors.white.withValues(alpha: 0.5) : const Color(0xFF94A3B8))),
                      );
                    },
                  ),
                ),
              ...
              lineBarsData: [
                LineChartBarData(
                  spots: dailyStats.asMap().entries.map((e) => FlSpot(e.key.toDouble(), e.value.maxPlayers.toDouble())).toList(),
                  isCurved: true,
                  ...
                  dotData: FlDotData(
                    show: true,
                    getDotPainter: (spot, percent, barData, index) => FlDotCirclePainter(radius: 4, color: const Color(0xFF3B82F6), strokeWidth: 2, strokeColor: isDark ? const Color(0xFF1E293B) : Colors.white),
                  ),
                  belowBarData: BarAreaData(
                    show: true,
                    gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [const Color(0xFF3B82F6).withValues(alpha: 0.3), const Color(0xFF3B82F6).withValues(alpha: 0.0)]),
                  ),
                ),
                LineChartBarData(
                  spots: dailyStats.asMap().entries.map((e) => FlSpot(e.key.toDouble(), e.value.avgPlayers.toDouble())).toList(),
                  isCurved: true,
                  ...
                  dotData: FlDotData(
                    show: true,
                    getDotPainter: (spot, percent, barData, index) => FlDotCirclePainter(radius: 4, color: const Color(0xFF10B981), strokeWidth: 2, strokeColor: isDark ? const Color(0xFF1E293B) : Colors.white),
                  ),
                  belowBarData: BarAreaData(
                    show: true,
                    gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [const Color(0xFF10B981).withValues(alpha: 0.2), const Color(0xFF10B981).withValues(alpha: 0.0)]),
                  ),
                ),
              ],
              lineTouchData: LineTouchData(
                touchTooltipData: LineTouchTooltipData(
                  ...
                  getTooltipItems: (touchedSpots) => touchedSpots.map((spot) {
                    ...
                    return LineTooltipItem('${isMax ? "最高" : "平均"}: ${spot.y.toInt()}', TextStyle(color: isMax ? const Color(0xFF3B82F6) : const Color(0xFF10B981), fontWeight: FontWeight.w600, fontSize: 12));
                  }).toList(),
                ),
              ),
            ),
...
class _HourlyChart extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    if (hourlyStats.isEmpty) return Center(child: Text('暂无数据', style: TextStyle(color: isDark ? Colors.white54 : Colors.black38)));
      ...
      children: [
        Row(
          ...
          children: [
            ...
            Text('峰值时段 $peakHour:00', style: TextStyle(fontSize: 11, color: isDark ? Colors.white70 : const Color(0xFF64748B))),
          ],
        ),
        const SizedBox(height: 8),
        Expanded(
            ...
            BarChartData(
              ...
              barTouchData: BarTouchData(
                touchTooltipData: BarTouchTooltipData(
                  getTooltipColor: (group) => isDark ? const Color(0xFF374151) : Colors.white,
                  tooltipRoundedRadius: 8,
                  getTooltipItem: (group, groupIndex, rod, rodIndex) => BarTooltipItem('${hourlyStats[groupIndex].hour}:00\n${rod.toY.toInt()} 人', TextStyle(color: isDark ? Colors.white : const Color(0xFF1E293B), fontWeight: FontWeight.w600, fontSize: 12)),
                ),
              ),
              titlesData: FlTitlesData(
                leftTitles: const AxisTitles(sideTitles: SideTitles(showTitles: false)),
                ...
                bottomTitles: AxisTitles(
                  sideTitles: SideTitles(
                    ...
                    getTitlesWidget: (value, meta) {
                      ...
                      return Padding(padding: const EdgeInsets.only(top: 8), child: Text('$hour', style: TextStyle(fontSize: 10, color: isDark ? Colors.white54 : const Color(0xFF94A3B8))));
                    },
                  ),
                ),
              ),
              gridData: FlGridData(
                ...
                drawVerticalLine: false,
                horizontalInterval: maxY / 4,
                getDrawingHorizontalLine: (value) => FlLine(color: isDark ? Colors.white.withValues(alpha: 0.1) : Colors.black.withValues(alpha: 0.05), strokeWidth: 1),
              ),
              borderData: FlBorderData(show: false),
              barGroups: hourlyStats.asMap().entries.map((e) {
                final isPeak = e.value.hour == peakHour;
                return BarChartGroupData(x: e.key, barRods: [BarChartRodData(toY: e.value.avgPlayers.toDouble(), color: isPeak ? const Color(0xFFEF4444) : const Color(0xFF8B5CF6), width: 8, borderRadius: const BorderRadius.vertical(top: Radius.circular(4)))]);
              }).toList(),
            ),
          ),
        ),
...
class _TopServersListState extends State<_TopServersList> {
  ...
  @override
  Widget build(BuildContext context) {
    if (widget.servers.isEmpty) return Center(child: Text('暂无数据', style: TextStyle(color: widget.isDark ? Colors.white54 : Colors.black38)));
      ...
      children: [
        ListView.separated(
          ...
          itemBuilder: (context, index) {
            ...
            final serverName = _serverNameCache[server.address] ?? server.address;

            return Row(
              children: [
                SizedBox(width: 20, child: Text('${index + 1}', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: index < 3 ? const Color(0xFFF59E0B) : (widget.isDark ? Colors.white54 : const Color(0xFF94A3B8))))),
                const SizedBox(width: 8),
                Expanded(
                  child: Stack(
                    children: [
                      Container(height: 28, decoration: BoxDecoration(color: widget.isDark ? Colors.white.withValues(alpha: 0.05) : Colors.black.withValues(alpha: 0.03), borderRadius: BorderRadius.circular(6))),
                      FractionallySizedBox(
                        widthFactor: widthRatio,
                        child: Container(height: 28, decoration: BoxDecoration(gradient: LinearGradient(colors: index < 3 ? [const Color(0xFFF59E0B).withValues(alpha: 0.3), const Color(0xFFF59E0B).withValues(alpha: 0.1)] : [const Color(0xFF8B5CF6).withValues(alpha: 0.3), const Color(0xFF8B5CF6).withValues(alpha: 0.1)]), borderRadius: BorderRadius.circular(6))),
                      ),
                      Positioned.fill(
                        child: Padding(
                          ...
                          child: Row(
                            children: [
                              Expanded(child: Text(serverName, style: TextStyle(fontSize: 11, color: widget.isDark ? Colors.white : const Color(0xFF1E293B)), overflow: TextOverflow.ellipsis)),
                              Text('峰值 ${server.maxPlayers} / 均值 ${server.avgPlayers}', style: TextStyle(fontSize: 10, color: widget.isDark ? Colors.white54 : const Color(0xFF94A3B8))),
                            ],
                          ),
                        ),
                      ),
        ...
        if (_canScrollUp)
          Positioned(top: 0, left: 0, right: 0, child: _buildScrollIndicator(isTop: true)),
        if (_canScrollDown)
          Positioned(bottom: 0, left: 0, right: 0, child: _buildScrollIndicator(isTop: false)),
      ],
    );
  }
  ...
  Widget _buildScrollIndicator({required bool isTop}) {
    ...
    return IgnorePointer(
      child: Container(
        ...
        child: Icon(isTop ? Icons.keyboard_arrow_up : Icons.keyboard_arrow_down, color: const Color(0xFF6B7280), size: 16),
      ),
    );
  }
}
...
class _TopMapsListState extends State<_TopMapsList> {
  ...
  @override
  Widget build(BuildContext context) {
    if (widget.maps.isEmpty) return Center(child: Text('暂无数据', style: TextStyle(color: widget.isDark ? Colors.white54 : Colors.black38)));
      ...
      children: [
        ListView.separated(
          ...
          itemBuilder: (context, index) {
            ...
            return Row(
              children: [
                SizedBox(width: 20, child: Text('${index + 1}', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: index < 3 ? const Color(0xFF10B981) : (widget.isDark ? Colors.white54 : const Color(0xFF94A3B8))))),
                const SizedBox(width: 8),
                Expanded(
                  child: ClipRRect(
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        ...
        Text(label, style: TextStyle(fontSize: 11, color: isDark ? Colors.white.withValues(alpha: 0.7) : const Color(0xFF64748B))),
      ],
    );
  }
}
