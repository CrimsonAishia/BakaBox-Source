...
class CharacterGalleryBloc extends Bloc<CharacterGalleryEvent, CharacterGalleryState> {
  ...
  Future<void> _onLoadCharacters(
    ...
    try {
      ...
      if (response != null) {
        ...
        emit(state.copyWith(
          listLoadState: LoadState.failure,
          error: '获取角色列表失败',
        ));
      }
    } catch (e) {
  ...
  Future<void> _onLoadCharacterDetail(
    ...
    try {
      ...
      if (character != null) {
        ...
        if (defaultSubModelId == null && character.subModels != null && character.subModels!.isNotEmpty) {
        ...
        if (character.category == CharacterCategory.touhou && defaultSubModelId != null) {
          add(LoadSpellCards(
  ...
  Future<void> _onSelectSubModel(
    ...
    if (currentSubModel?.preview == null) {
      try {
        final subModelDetail = await _api.getSubModelDetail(character.id, event.subModelId);
  ...
  Future<void> _onLoadSpellCards(
    ...
    try {
      final result = await _api.getSpellCards(event.characterId, subModelId: event.subModelId);
  ...
  Future<void> _onSubmitUnifiedEdit(
    ...
    try {
      ...
      if (event.spellCardCreates != null || event.spellCardUpdates != null || event.spellCardDeletes != null) {
      ...
      if (event.zombieSkillCreates != null || event.zombieSkillUpdates != null || event.zombieSkillDeletes != null) {
  ...
  Future<void> _onLoadSpellCardTierList(
    ...
    try {
      final response = await _api.getSpellCardTierList(type: event.type);
      if (response != null) {
        // 默认展开第一个梯队
  ...
  Future<void> _onNavigateToCharacterFromSpellCard(
    ...
    // 保持符卡梯队视图，只在右侧加载角色详情，并记录选中的符卡ID
    emit(state.copyWith(
    ...
    try {
      ...
      if (character != null) {
        ...
        if (targetSubModelId == null) {
          ...
          if (targetSubModelId == null && character.subModels != null && character.subModels!.isNotEmpty) {
            final defaultSubModel = character.subModels!.firstWhere(
              (s) => s.isDefault,
              orElse: () => character.subModels!.first,
            );
        ...
        if (character.category == CharacterCategory.touhou && targetSubModelId != null) {
          add(LoadSpellCards(
            characterId: character.id,
  ...
  void _onToggleTierExpanded(
    ...
    final newExpandedTiers = Set<String>.from(state.expandedTiers);
    if (newExpandedTiers.contains(event.tier)) {
      newExpandedTiers.remove(event.tier);
    } else {
      newExpandedTiers.add(event.tier);
    }
    emit(state.copyWith(expandedTiers: newExpandedTiers));
  }
}
