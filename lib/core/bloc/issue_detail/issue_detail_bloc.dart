...
class IssueDetailBloc extends Bloc<IssueDetailEvent, IssueDetailState> {
  ...
  Future<void> _onLoadComments(
    ...
    try {
      final response = await _issueApi.getComments(
        event.issueId,
        page: 1,
        pageSize: 10,
      );
  ...
  Future<void> _onLoadMoreComments(
    ...
    try {
      ...
      final newItems = response.items
          .where((newItem) => !state.comments.any((c) => c.id == newItem.id))
          .toList();
      ...
      emit(
        state.copyWith(isLoadingMoreComments: false, hasLoadMoreError: true),
      );
    }
  }
