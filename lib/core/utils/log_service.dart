...
class LogService {
  ...
  static Future<void> _openLogFile() async {
    ...
    final logFile = '${AppDirectoryService.logsPath}${Platform.pathSeparator}$today.log';
  ...
  static void _flushQueueToFile() {
    ...
    Future.microtask(() async {
      try {
        ...
        // 检查是否需要切换日志文件（跨天）
        final today = _fileDateFormat.format(DateTime.now());
        final expectedLogFile = '${AppDirectoryService.logsPath}${Platform.pathSeparator}$today.log';
        ...
        await for (final entity in logsDir.list()) {
          if (entity is File && entity.path.endsWith('.log')) {
            try {
              ...
              final dateMatch = RegExp(r'(\d{4}-\d{2}-\d{2})\.log$').firstMatch(fileName);
