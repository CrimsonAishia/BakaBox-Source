...
///
/// 架构说明（重要）：
/// - **主要的单实例拦截已下沉到原生层**（windows/runner/main.cpp）：
///   第二个进程会在「创建窗口、启动 Flutter 引擎之前」通过命名互斥体检测到
///   已有实例并立即 `return`，因此不会再拉起整个引擎、不会在任务管理器残留
///   半启动的进程。原生层随后向命名管道写入数据，触发首个实例唤醒窗口。
/// - **本服务在首个实例中仍然必不可少**：它通过 windows_single_instance
///   插件创建命名管道并监听 [_wakeExistingInstance]，正是原生第二实例写入的
///   目标管道。它同时作为兜底，确保即使原生守卫未生效也能退出重复实例。
///
/// 设计要点：
