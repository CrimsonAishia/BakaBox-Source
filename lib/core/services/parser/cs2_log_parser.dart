...
class CS2LogParser {
  ...
  static final _regConnecting = RegExp(r'\[Client\]\s+Sending connect to\s+(.*)');
  ...
  static final _regServerFullFallback = RegExp(r'(?:[Ss]erver is full|[Nn]o free slots|SERVERFULL|REJECT_SERVERFULL)');
  
  // Fully spawned and in-game markers
  static final _regSpawned = RegExp(r'\[Prediction\]\s+Added prediction for player|ChangeGameUIState:.*LOADINGSCREEN\s*->\s*CSGO_GAME_UI_STATE_INGAME');

  // Connection established (but still loading)
  static final _regConnected = RegExp(r"\[Client\]\s+CL:\s+Connected to\s+'?([^']+)'?");
  ...
  static CS2EngineEvent? parse(String line) {
    ...
    if (signonMatch != null) {
      return EvSignonState(int.parse(signonMatch.group(1)!), signonMatch.group(2)!);
    }
