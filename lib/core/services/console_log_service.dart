...
class ConsoleLogService {
  ...
  final _regexConnectFailed = RegExp(
    r'[Cc]onnection failed|[Ff]ailed to connect|[Uu]nable to connect',
  );

  // 连接阶段断开检测（关键：在连接过程中被断开）
  final _regexDisconnectionDuringConnection = RegExp(
    r'Disconnection during connection phase',
  );

  // 连接超时断开检测（在连接阶段因超时被断开）
  final _regexDisconnectedTimedout = RegExp(
    r'Disconnected from server:\s*NETWORK_DISCONNECT_TIMEDOUT',
  );

  // 断开原因检测
  final _regexDisconnectLoopShutdown = RegExp(
    r'NETWORK_DISCONNECT_LOOPSHUTDOWN',
  );
  ...
  final _regexMapLoaded = RegExp(
    r'ChangeGameUIState:.*CSGO_GAME_UI_STATE_LOADINGSCREEN',
  );

  // 服务器状态
  final _regexServerFull = RegExp(
  ...
  // 连接超时检测
  final _regexConnectTimeout = RegExp(
    r'CONNECT_REQUEST_TIMEDOUT|[Tt]imed out attempting to connect',
  );
  ...
  // 主菜单状态检测
  final _regexMainMenu = RegExp(
    r'ChangeGameUIState:.*CSGO_GAME_UI_STATE_MAINMENU',
  );
  ...
  Future<ConnectionStatusResult> monitorConnection({
    ...
    _checkTimer = Timer.periodic(const Duration(milliseconds: 500), (
      timer,
    ) async {
      if (_isCancelled) {
        timer.cancel();
  ...
  Future<void> _analyzeHistoryAndRestoreState(File file) async {
    try {
      ...
      try {
        ...
        // 用于检测远程连接的正则
        final regexHostActivate = RegExp(
          r'\[HostStateManager\] Host activate: Remote Connect \(([^)]+)\)',
        );
        ...
        for (int i = lines.length - 1; i >= 0; i--) {
          ...
          if (remoteConnectIndex == null) {
            ...
            if (hostMatch != null) {
              ...
              LogService.d(
                '[ConsoleLog] 历史分析: 检测到远程连接 @$i: $lastServerAddress',
              );
            }
          }
        ...
        if (isInGame && lastServerAddress != null) {
          ...
          _isInLoopbackMode = false;
          LogService.d(
            '[ConsoleLog] 历史: 用户当前在服务器中: $lastServerAddress, 地图: $lastMapName',
          );
        } else {
          _currentState = _currentState.copyWith(
            state: GameState.mainMenu,
          ...
          LogService.d(
            '[ConsoleLog] 历史: 用户当前在主菜单, loopbackMode=$_isInLoopbackMode',
          );
        }
      } finally {
        await raf.close();
  ...
  void _parseLine(String line) {
    ...
    if (_regexDisconnectionDuringConnection.hasMatch(line)) {
      // 检查具体的断开原因
        ...
        // LOOPSHUTDOWN 是游戏引擎在切换连接状态时的正常行为
        // 通常发生在重连过程中，不应该标记为失败
        LogService.d('[ConsoleLog] 检测到连接阶段断开 (LOOPSHUTDOWN - 正常重连流程)');
      } else {
        // 其他未知的断开原因，标记为失败
        LogService.d('[ConsoleLog] 检测到连接阶段断开 (未知原因): $line');
        _connectionPhaseFailed = true;
      }
      return;
    }
    ...
    if (_regexLoopbackConnect.hasMatch(line)) {
      ...
      if (_targetServer.isNotEmpty &&
          (_connectTimedOut || _connectionPhaseFailed)) {
        ...
        _updateConnectionState(
          ...
          _currentState.state == GameState.loading) {
    ...
    if (connectedMatch != null) {
      ...
      if (connectedTo.toLowerCase().contains('loopback')) {
        if (_targetServer.isNotEmpty && _connectTimedOut) {
          _isLoopbackFallback = true;
          LogService.d(
            '[ConsoleLog] 检测到连接到 loopback，目标服务器连接失败: $_targetServer',
          );
          return;
        }
      }
    ...
    if (_regexLoadingToIngame.hasMatch(line)) {
      ...
      if (currentState == GameState.loading && !_isLoopbackFallback) {
        LogService.d(
          '[ConsoleLog] 检测到 LOADINGSCREEN -> INGAME，等待 Prediction 确认',
        );
    ...
    if (_regexRetrying.hasMatch(line)) {
      _updateConnectionState(GameState.retrying, rawLine: line);
      return;
    }
    ...
    if (_regexDisconnect.hasMatch(line)) {
      ...
      if (!line.contains('LOOPSHUTDOWN') &&
          !line.contains('LOOPDEACTIVATE') &&
          !line.contains('DISCONNECT_BY_USER')) {
        // 已在上面处理
        final currentState = _currentState.state;
        if (currentState == GameState.inGame) {
          _updateConnectionState(GameState.mainMenu, serverAddress: '', rawLine: line);
