...
final RegExp vpkInnerRe = RegExp(
  r'\.vpk:([A-Za-z0-9_/\\\-\.]+?\.(?:' + _vpkInnerSuffixes.join('|') + r'))\b',
  caseSensitive: false,
);
...
const List<String> fatalKeywords = [
  'FATAL ERROR',
  'attempting to render with error material',
  'failed to load',
  'unable to find',
  'unable to load',
  'PoseRecipe',
...
const Map<String, ThirdPartyHint> thirdPartyHints = {
  // 良性
  'gameoverlayrenderer': ThirdPartyHint(
    'benign',
    'Steam 游戏内覆盖层 (Shift+Tab / 帧数显示)',
    'Steam 官方组件, 正常注入, 一般无需处理',
  ),
  'steamoverlayvulkanlayer': ThirdPartyHint(
    'benign',
    'Steam Vulkan 覆盖层',
    'Steam 官方组件, 正常',
  ),
  'gameoverlayrenderer64': ThirdPartyHint(
    'benign',
    'Steam 游戏内覆盖层 (Shift+Tab / 帧数显示)',
    'Steam 官方组件, 正常注入, 一般无需处理',
  ),
  ...
  'msi.dll': ThirdPartyHint(
    'medium',
    'MSI Afterburner',
    '部分版本与 CS2 不兼容, 建议升级或关闭',
  ),
  'afterburner': ThirdPartyHint(
    'medium',
    'MSI Afterburner',
    '部分版本与 CS2 不兼容, 建议升级或关闭',
  ),
  'discord': ThirdPartyHint('medium', 'Discord 覆盖层', '偶尔引发崩溃, 可临时关闭覆盖层'),
...
const Map<String, (String, List<String>)> crashModuleProfile = {
  ...
  'resourcesystem.dll': (
    '资源加载',
    ['vmdl_c', 'vmap_c', 'vmat_c', 'vsnd_c', 'vpcf_c'],
  ),
  'schemasystem.dll': ('数据模式', []),
