...
String renderToString(Map<String, dynamic> r, {bool verbose = false}) {
  ...
  p('  异常码    : 0x${(exc['code'] as int).toRadixString(16).toUpperCase().padLeft(8, '0')}  -> ${exc['code_name']}');
  p('  异常地址  : ${fmtAddr(exc['address'] as int)}');
  if (exc.containsKey('av_op')) {
    var line = '  访问类型  : ${exc['av_op']} 目标 ${fmtAddr(exc['av_target'] as int)}';
  ...
  if (crashModule != null) {
    final prof = crashModuleProfile[(crashModule['name'] as String).toLowerCase()];
    final tag = prof != null ? '  [${prof.$1}]' : '';
    p('  崩溃位置  : ${crashModule['name']}+0x${_hex(crashModule['rva'] as int)}$tag');
  } else {
    p('  崩溃位置  : <非已加载模块>');
  }
  ...
  if (regStrings.isNotEmpty) {
    ...
    regStrings.forEach((rn, info) {
      ...
      p('  $rn -> 0x${_uhex(info['addr'] as int).padLeft(16, '0')}  ${_repr(txt)}');
    });
  }
  ...
  if (resources.isNotEmpty) {
    ...
    final kindOrder = [
      'vmap_c', 'vmap', 'vwnod_c', 'vwnod',
      'vmdl_c', 'vmdl',
      'vmat_c', 'vmat', 'vpcf_c', 'vpcf',
      'vtex_c', 'vsnd_c', 'vsnd', 'vsndevts_c', 'vsndevts',
      'vanim_c', 'vphys_c', 'vphys', 'vnmgraph_c',
      'vrman_c', 'vpk', 'other'
    ];
    ...
    for (final kind in kindOrder) {
      ...
      final limit = verbose ? items.length : (items.length < 8 ? items.length : 8);
  ...
  // --- 调用链 ---
  section('调用链 (核心模块)');
  final coreChain = (r['call_chain'] as List)
      .where((c) => c['is_core'] == true)
      .toList()
    ..sort((a, b) =>
  ...
  if (coreChain.isNotEmpty) {
    ...
    for (final c in coreChain.take(limit)) {
      p('  +${_off5(c['stack_offset'] as int)}  ${c['module']}+0x${_hex(c['rva'] as int)}');
    }
  ...
  if (subsystems.isNotEmpty) {
    ...
    for (final sub in subsystems.take(8)) {
      p('  ${(sub['module'] as String).padRight(30)}  x${(sub['hits'] as int).toString().padRight(4)}  ${sub['label']}');
    }
  }
  ...
  if (verbose) {
    ...
    for (final e in hits.take(15)) {
      final flag = coreModules.contains((e.key as String).toLowerCase()) ? ' *core' : '';
  ...
  if (tpm.isNotEmpty) {
    ...
    final mods = List.from(tpm)
      ..sort((a, b) =>
          (order[a['sev']] ?? 1).compareTo(order[b['sev']] ?? 1));
  ...
  if (deep != null) {
    ...
    if (objVtables.isNotEmpty) {
      ...
      objVtables.forEach((rn, info) {
        p('    $rn=0x${_uhex(info['obj_addr'] as int)}  -> vtable ${info['vtable']}');
      });
    }
    ...
    if (vtables.isNotEmpty) {
      ...
      for (final vt in vtables.take(8)) {
        p('    x${(vt['count'] as int).toString().padRight(4)} ${vt['fingerprint']}');
      }
...
String _off5(int v) =>
    '0x${v.toRadixString(16).toUpperCase().padLeft(5, '0')}';
